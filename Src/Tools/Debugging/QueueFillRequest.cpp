...
In& operator>>(In& stream, QueueFillRequest& queueFillRequest)
{
	...
	unsigned char temp;
	stream >> temp;   queueFillRequest.behavior = static_cast<QueueFillRequest::Behavior>(temp);
	stream >> temp;   queueFillRequest.filter = static_cast<QueueFillRequest::Filter>(temp);
	stream >> temp;   queueFillRequest.target = static_cast<QueueFillRequest::Target>(temp);
	stream >> temp;   // padding
