                          ...
                          )
{
  ...
  for (i=0; i< numberOfStates; i++)
  {
    ...
    states.append(stateName, new State(stateName,errorHandler,pTimeFunction));
  }
  ...
  for (i=0; i< numberOfStates; i++)
  {
    states[i]->create(input, options, basicBehaviors, states, static_cast<OptionParameters&>(*parameters), symbols, timeOfExecution);
