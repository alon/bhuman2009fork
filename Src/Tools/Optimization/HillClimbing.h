...
#ifndef __HillClimbing_h_
#define __HillClimbing_h_
...
{
public:
  enum Mode {MINIMIZE, MAXIMIZE} mode;

private:
  bool initialized;
  bool newHillDuringLastExecution;
  int localMinimumCountingLimit;
...
public:
  ...
  void init(T startValues[N], T minValues[N], T maxValues[N], T maxStepSize[N],
    int limit, R worstReward, Mode mode, int numOfRuns, int maxUnchangedSteps)
  {
    for(int i=0; i<N; ++i)
    {
    ...
    this->mode = mode;
    localMinimumCountingLimit = limit;
...
//      maxUnchangedSteps/=2;
  ...
  void getNextParameterSet(T newValues[N])
  {
    for(int i=0; i<N; ++i)
    {
      T delta = static_cast<T>(2.0*(0.5 - randomDouble()) * maxStepSize[i]);
